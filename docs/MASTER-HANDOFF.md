...
> **This repository is AVINEH, derived from PARNIAN.** Everything below is
> inherited unchanged except three things re-decided for a bridal atelier:
> the catalogue ratio is `3/4` rather than `4/5` because a gown is read
> standing; the section rhythm is roughly 20% airier because a gown needs
> room above and below it; and each item carries optional `views` for the
> extra angles a gown buyer expects, which PARNIAN had no need of. Wherever
> this document says «cosmetics», «product» or «پرنیان», read «atelier»,
> «gown» and «آوینه». §41, §42, §44.1, §52 and §53 apply verbatim.
